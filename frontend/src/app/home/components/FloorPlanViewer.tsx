...
const FloorPlanViewer: FC<FloorPlanViewerProps> = ({ originalData }) => {
  ...
  const getFloorSize = useCallback((floor: FloorData) => {
    const maxX = Math.max(
      ...floor.rooms.map((r) => r.x + r.width),
      0,
      ...floor.objects.map((o) => o.x + o.width)
    );
    const maxY = Math.max(
      ...floor.rooms.map((r) => r.y + r.height),
      0,
      ...floor.objects.map((o) => o.y + o.height)
    );
  ...
  const drawAllFloorPlans = useCallback(() => {
    ...
    const drawFloor = (floorNum: 1 | 2, baseX: number, baseY: number) => {
      ...
      if (!floor) return;
      const elements = [...floor.rooms, ...floor.objects].filter(
        (el): el is PlanElement => !!el
      );
      ...
      elements.forEach((el) => {
        ...
        if (el === selectedElement && type === "object") {
          ctx.fillRect(x + w - 6, y + h - 6, 6, 6); // 리사이즈 핸들
        }
      });
    };

    drawFloor(1, floor1OffsetX, floor1OffsetY);
    if (hasSecondFloor) drawFloor(2, floor2OffsetX, floor2OffsetY);
  ...
  // objects만 클릭 가능 (1층 + 2층)
  const getClickedElement = (mx: number, my: number): PlanElement | null => {
    const floors: [FloorData, number][] = [
      [data[1], dimensions.floor1OffsetX],
      [data[2]!, dimensions.floor2OffsetX],
    ].filter(Boolean) as [FloorData, number][];
